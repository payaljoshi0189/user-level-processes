	...
	-- Initialize the Condition variables
	 ...
	 processManagerLock.Lock ()				-- Acquire the Monitor's mutex lock to have exclusive access to monitor's entry method
	  ...
	  aProcessBecameFree.Wait (&processManagerLock)
         ...
         p.myThread = currentThread				-- newly added code, connect the PCB to thread
         --self.Print ()
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    --
    -- Read a new executable program from disk and copy it into 
    -- the address space of the process which invoked the Exec
    -- Begin execution of the new program
    -- Overwrites the address space of the parent process
    ...
    var 
         ...
         oldAddrSpacePtr : ptr to AddrSpace   
    ...
    if openFile != null
       oldAddrSpacePtr = &currentThread.myProcess.addrSpace

       newAddrSpace = new AddrSpace
       newAddrSpace.Init()     

       -- Create a virtual address space and
       -- load the executable program into new address space
       initPC = openFile.LoadExecutable (&newAddrSpace)
     
       -- Copy the new address space into the process control block
       currentThread.myProcess.addrSpace = newAddrSpace


       -- Free up the previous address space
       frameManager.ReturnAllFrames (oldAddrSpacePtr)

       -- Close the executable file
       fileManager.Close (openFile)

       -- Calculate the stacktop address
       initUserStackTop = 0 + (currentThread.myProcess.addrSpace.numberOfPages * PAGE_SIZE)

       -- Reset the system stack
       initSystemStackTop = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger

       -- Turn the thread into user level thread
       oldStatus = SetInterruptsTo (DISABLED)

       -- Initialize the page table registers for this logical address space
       currentThread.myProcess.addrSpace.SetToThisPageTable ()

      -- Set the isUserThread variable in the current thread to true
       currentThread.isUserThread = true
         
       --oldStatus = SetInterruptsTo(ENABLED)
       -- Change from system-mode to user-mode
       -- Enable interrupts and paging
       -- Jump into the user-level program
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      print ("filename = ")
      -- Copy filename string from virtual address space into the kernel address space
      i = currentThread.myProcess.addrSpace.GetStringFromVirtual (&strBuffer, filename asInteger , MAX_STRING_SIZE)
...
-----------------------------  OpenFile  ---------------------------------
  ...
  ------------------------------- InitFirstProcess --------------------------------------
    function InitFirstProcess ()
       --
       -- This function calls ThreadManager's GetANewThread () method to
       -- request for a resource thread, initializes the Thread returned by ThreadManager
       -- and calls a Fork on it to execute 'StartUserProcess' function
       --
     ...
     function StartUserProcess (p: int)
       --
       -- Newly acquired 'UserProgram' thread executes this function
       -- This function allocates a process control block 'pcb' to the 'UserProgram' thread
       -- Next it finds a pointer to the File Control Block that points to UserProgram, opens a file
       -- with the help of File Manager and returns a pointer to newly allocated OpenFile.
       -- Load the executable program into newly created virtual address space
       -- Point the User's stack top to appropriate location, reset the system stack
       -- Jump to User-level Program to start its execution
       --
	 ...
	 -- Create a virtual address space and 
         -- load the executable program into new address space
         initPC = openFile.LoadExecutable (addrPtr)

         -- Close the executable file
         fileManager.Close (openFile)
	 
         -- Calculate the stacktop address
	 initUserStackTop = 0 + (addrPtr.numberOfPages * PAGE_SIZE)
